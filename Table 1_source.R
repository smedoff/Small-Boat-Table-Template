...
# Table Template: Percentage of ranges mean and median by respondent group
# Author: Sarah Medoff

#====================

# Notes 
# - not sure what "total base shuld be xxx" refers to for CatchPerTrip and 
#   ValuePerTrip
#   These tables do not have templates so I cannot check if the calc were right
...
# Set parameters for each table produced with this table template
  ...
  table_details_master.l <- lapply(1:length(input_variable.l), FUN = function(i){
    ...
    one_sheet.l <- list(
      table_variable = unique(one_sheet.df$variable),
...
# Loop through tables 
  ...
  table.l <- list()
  table_names.v <- rep(NA, length(table_details_master.l))
  for(i in 1:length(table_details_master.l)){
    
    print(i)
    
    one_table_details.l <- table_details_master.l[[i]]
    table_names.v[i] <- names(table_details_master.l)[i]
    
    #------------
    # Store all variable values 
    table_var <- one_table_details.l[["table_variable"]]
    breaks.v <- one_table_details.l[["binned_values"]]
    labels.v <- one_table_details.l[["label_values"]]
    ...
    # Compiling table
    TABLE01 <- compile_one_table1.f()
    
    # Remove NAs from output 
    
    
    # Saving table to appropriate folder
    saving_final_data.f(final_data = TABLE01)
  ...
  names(table.l) <- table_names.v
  
#----------
# Code check 
  # We can check the tables manually 
  
  View(table.l[["Q21.1xVALUE"]])
  View(table.l[["TOTALCaught"]])
  View(table.l[["Q20xMEAN"]])
