...
compute_one_block_table03b.f <- function(df,
                                         type){
  ...
  COL.df <- data.frame(BANNER_VAR = as.character(binned.v),
                       VARIABLE_GROUPS = label.v)
  ...
  if(nrow(df_categories_defined) == 0){
    ...
    df_group_stats <- df_categories_defined %>% 
      mutate(BANNER_VAR = as.character(BANNER_VAR)) %>% 
      ...
      group_by(VARIABLE_GROUPS) %>% 
      dplyr::summarize(PERC = sum(PERC, na.rm = TRUE))
...
#    }
    ...
    one_table_block <- data.frame(TYPE = type) %>% 
      cbind(spread(df_group_stats_FULL, VARIABLE_GROUPS, PERC)) %>% 
      select(TYPE, "Number of Respondents", label.v)
