...
# Table Template: Mean SE and Median for question series with multiple questions in banner
# Author: Sarah Medoff
...
# Setting table number 
  ...
  source(file.path(".", "helper function", 
                   "Table Specific",
                   "Table 9c_hlpfnx_compile one table block.R")) 
...
# Set parameters for LHS variables 
  ...
  for(i in 1:length(table_details_master.l)){
    ...
    for(j in 1:length(LHS_VARS.l)){
      ...
      TABLE09c.l <- lapply(1:nrow(VARS.df), 
                           FUN = compute_one_block_table09c.f,
                           data.df = HI_SMALL_BOAT.df, 
                           var.df = VARS.df)
      
      
      TABLE09c <- rbind(do.call(rbind, TABLE09c.l))
      
      # Saving table to appropriate folder
      saving_final_data.f(final_data = TABLE09c)
    }
